...
def plot(fig_num, outfile):
    """
    Plot the median housing price and number of homicides
    by neighborhood.
    """
    # load the data
    ...
    with plt.style.context("fivethirtyeight"):
        ...
        fig, axs = plt.subplots(
            nrows=nrows, ncols=ncols, figsize=(6.45, 10), gridspec_kw=grid_kws
        )
        ...
        for i in range(nrows):
            for j in range(ncols):
                ax = axs[i, j]
                if cnt >= len(neighborhoods):
                    ax.set_axis_off()
                    continue

                hood = neighborhoods[cnt]
                ax2 = ax.twinx()
